...
PKEY_VAR_CPU_UTIL_SHOW = 'cpu.util.show'
...
#                     self.logger.debug("up: %s, %s"%( str(_rcnt), str(_rid) ))
                if not _res:
                    self.logger.error("FAIL: %s Info DB-Update Error, err=%s, sql=%s, param=%s"%( self.ltt, str(_rcnt), sql, str(_p) ))
                else:
                    if _rcnt < 1 :
                        _res = True
                        try:
...
#                             self.logger.debug("in: %s, %s"%( str(_rcnt), str(_rid) ))
                        if not _res:
                            self.logger.error("FAIL: %s Info DB-Insert Error, err=%s, sql=%s, param=%s"%( self.ltt, str(_rcnt), sql, str(_p) ))
...
def get_steps(_rkey, _step3=None, _step2=None):
    ...
    #   CS AMF/SMF/UPF
        ...
        if str(_rkey).find(PKEY_VCS_MEM_USE_SHOW) > 0 :
            return True, [_st1, _step2, _step3, _rkey], None
    ...
    #   ARIEL
         ...
         if PKEY_VAR_VNF_STATUS in _rkey:
            ...
            return True, [_st1, ST2_PORT, _step3, _rkey], None
         elif PKEY_VAR_CPU_UTIL_SHOW in _rkey:
            return True, [_st1, ST2_CPU, ST3_UTIL, _rkey], None
